...
class UpdatePatientDialog(QDialog):
    ...
    def set_data(self, data: dict):
        self.name_input.setText(data["full_name"])

        date = QDate.fromString(data["birthday"], "yyyy-MM-dd")
        if date.isValid():
            self.date_input.setDate(date)
        else:
            self.date_input.setDate(QDate.currentDate())  # ili ostavi prazno, kako želiš

        self.gender_input.setCurrentText(data["gender"])
        self.address_input.setText(data["address"])
        self.phone_input.setText(data["phone_number"])
